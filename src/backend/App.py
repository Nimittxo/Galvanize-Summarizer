...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Or ["http://localhost:3000"] if that's your React dev server
...
def summarize(req: SummarizeRequest):
    ...
    prompt = (
        ...
        "Each bullet point should be a complete sentence and cover a different important aspect of the text:\n"
        + text
    )
    summary = summarizer(prompt, max_length=200, min_length=40, do_sample=False)
    summary_text = summary[0]['summary_text']

    # Bulletize using nltk sentence tokenizer
